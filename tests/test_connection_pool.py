...
class TestConnectionPoolUnixSocketURLParsing:
    ...
    def test_connection_class_override(self):
        class MyConnection(valkey.UnixDomainSocketConnection):
            pass
...
class TestMultiConnectionClient:
    @pytest.fixture()
    def r(self, request):
        return _get_client(valkey.Valkey, request, single_connection_client=False)
...
class TestHealthCheck:
    ...
    @pytest.fixture()
    def r(self, request):
        return _get_client(valkey.Valkey, request, health_check_interval=self.interval)
